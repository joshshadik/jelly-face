...
function tick(a){stats.begin();_time.update(a);isLoading&&_firstUpdate||!_started&&!_firstUpdate||(resize(),_jellyFace.update(),render(),_jellyFace.postUpdate(),_firstUpdate=!1);if(isLoading){currLoad=(currLoad+Time.deltaTime())%1;a=Math.floor(4*currLoad);for(var b="l o a d i n g ",c=0;c<a;++c)b+=". ";loadingElement.innerText=b;_firstUpdate&&(gl.clearColor(.9,.9,.9,0),gl.clear(gl.COLOR_BUFFER_BIT))}stats.end();requestAnimationFrame(tick)}
...
3==touches.length,.03*d);return!1}var Framebuffer=function(a,b,c,d){this._fbo=gl.createFramebuffer();this.setup(a,b,c,d)};
